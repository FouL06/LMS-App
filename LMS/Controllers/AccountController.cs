...
namespace LMS.Controllers
{
    ...
    public class AccountController : CommonController
    {
        ...
        public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
        {
            ...
            //Get top 3 Uid's to compare
            var getTopAdmin = (from a in db.Administrators
                               orderby a.UId descending
                               select a.UId).FirstOrDefault()?.ToString();

            if(!(getTopAdmin is null))
            {
                newUid = getTopAdmin;
            }

            var getTopProf = (from p in db.Professors
                              orderby p.UId descending
                              select p.UId).FirstOrDefault()?.ToString();

            if(!(getTopProf is null))
            {
                if(string.Compare(getTopProf, newUid) == 1)
                {
                    newUid = getTopProf;
                }
            }

            var getTopStud = (from s in db.Students
                              orderby s.UId descending
                              select s.UId).FirstOrDefault()?.ToString();

            if (!(getTopStud is null))
            {
                if (string.Compare(getTopStud, newUid) == 1)
                {
                    newUid = getTopStud;
                }
            }

            //Iterate the top found Uid for the new Uid to be set
            int id = int.Parse(newUid.Substring(1));
            id++;
            newUid = "u" + id.ToString().PadLeft(7, '0');

            //Insert user data into correct table based on role
